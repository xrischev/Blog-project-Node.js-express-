...
module.exports = {
    ...
    details: (req, res) => {
        ...
        Article.findById(id).populate('author tags').then(article => {
            ...
            Comment.find({'article':mongoose.Types.ObjectId(id)}).populate('author').then(comment=>{
            ...
            req.user.isInRole('Admin').then(isAdmin => {
                ...
                res.render('article/details', { article: article,comment:comment, isUserAuthorized: isUserAuthorized});
            });
        });
    ...
    editPost: (req, res) => {
        ...
        if (errorMsg) {
            ...
            Article.findById(id).populate('category tags').then(article => {
                ...
                /*article.save((problem123) => {
                    if (problem123) {
                        console.log(problem123.message)
                    }*/
    ...
    deletePost: (req, res) => {
        ...
        Article.findById({_id:id}).then(article=>{
           for(let comment of article.coment){


               Comment.findById({_id:comment}).remove().then(coment=>{
               })
           }
        })

        Liked.find({'article':id}).then(liked=>{
            console.log(liked)
